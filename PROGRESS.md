...
## M2: 生成系 — 実装完了(実機AI受入は CLAUDE_CODE_OAUTH_TOKEN 設定後)
...
- [ ] 受入(実機): 技生成→戦闘使用→アップデートの一連。敵AIのヒーラー狙い+挑発ロック遵守の実演
      ※AI実行には リポジトリSettings→Secrets→Actions に CLAUDE_CODE_OAUTH_TOKEN の設定が必要
      (未設定でもフォールバックでゲームは動く。トークンは `claude setup-token` で発行)
- [ ] gitタグ v0.2-m2(実機受入後)
