...
def test_all_normal_is_valid(battle_save):
    errors = validate_commands(battle_save, battle_save.battle, all_normal_commands(), ULT_MAX)
    assert errors == []


def test_ability_on_ct_rejected(battle_save):
    ...
    errors = validate_commands(battle_save, battle_save.battle, cmds, ULT_MAX)
    assert len(errors) == 1
    assert errors[0].role == "attacker"
    assert "CT中" in errors[0].reason


def test_ult_without_gauge_rejected(battle_save):
    cmds = all_normal_commands()
...
def test_ult_with_full_gauge_ok(battle_save):
    battle_save.member_by_role("attacker").ult_gauge = 100
    cmds = all_normal_commands()
    cmds["attacker"] = Command("attacker", "奥義", "自動")
    assert validate_commands(battle_save, battle_save.battle, cmds, ULT_MAX) == []


def test_nonexistent_enemy_target_rejected(battle_save):
    cmds = all_normal_commands()
...
def test_attack_on_ally_rejected(battle_save):
    cmds = all_normal_commands()
...
def test_heal_on_enemy_rejected(battle_save):
    cmds = all_normal_commands()
...
def test_unknown_action_rejected(battle_save):
    cmds = all_normal_commands()
...
def test_missing_command_rejected(battle_save):
    cmds = all_normal_commands()
    del cmds["tank"]
    errors = validate_commands(battle_save, battle_save.battle, cmds, ULT_MAX)
    assert len(errors) == 1
...
def test_dead_member_command_not_error(battle_save):
    battle_save.member_by_role("support").hp = 0
    errors = validate_commands(battle_save, battle_save.battle, all_normal_commands(), ULT_MAX)
    assert errors == []


def test_wait_ignores_target(battle_save):
    cmds = all_normal_commands()
